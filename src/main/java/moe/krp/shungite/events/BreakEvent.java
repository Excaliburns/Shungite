...
public class BreakEvent {
    ...
    public static void breakFaster(final PlayerEvent.BreakSpeed event) {
        ...
        final MobEffectInstance effectInstance = player.getEffect(ModEffects.DEXTEROUS.get());

        // duplicate aqua affinity logic
        // If they don't have aqua affinity, but they do have the effect, give them a boost :)
        if (player.isInFluidType() && !EnchantmentHelper.hasAquaAffinity(player) && effectInstance != null) {
            int effectLevel = effectInstance.getAmplifier();
            float f1 = event.getOriginalSpeed();
            event.setNewSpeed(f1 * (5.0F * effectLevel));
        }
    }
}
