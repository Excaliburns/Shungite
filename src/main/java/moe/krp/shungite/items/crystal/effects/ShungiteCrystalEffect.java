...
public class ShungiteCrystalEffect {
  ...
  public static ShungiteCrystalEffect deserialize(final CompoundTag tag) {
    ...
    final ShungiteEffect tagStat = Registration.SHUNGITE_EFFECT_DEFERRED_REGISTER
        .getEntries()
        .stream()
        .filter( any -> any.get().getName() == tagString)
        .findFirst()
        .orElseThrow( () -> new IllegalStateException("Invalid ShungiteEffect deserialized from NBT."))
        .get();
  ...
  public static List<MutableComponent> listToString(final List<ShungiteCrystalEffect> stats) {
    ...
    textComponents.add(
        ShungiteUtils.getTextComponent("info.shungite.stats")
            .append(":")
    ...
    for (ShungiteCrystalEffect s : stats) {
      final MutableComponent statComponent = Component.literal(
          s.getCrystalEffect().getMobEffect().getDisplayName().getString()
      );
      ...
      statComponent
          .append("(")
          .append(Integer.toString(s.crystalEffect.getPointValue() * (s.level + 1)))
          .append(")");
